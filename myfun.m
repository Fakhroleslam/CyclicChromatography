...
function dudt=myfun(t,yin)
global nx epsilon kA  v Dax delz kpa AdsorpQ A cA0
dudt=zeros(2*nx,1);
dudt=dudt';
for i= 2:nx-1
    if i==2
    ...
    if i==nx-1
        yin(nx)=(4/3)*(yin(nx-1))-(1/3)*(yin(nx-2));
    end
    ...
    dudt(i+nx+1)=kpa*(qstar(i)-yin(i+nx+1));
    dudt(i)=(Dax*d2cA_dz2(i))-v/epsilon*dcA_dz(i)-((1-epsilon)/epsilon)*dudt(i+nx+1);
end
end